...
const NAME_RULES = [
  // HIGH
  { pattern: /\b(ssd|nvme|m\.2|thumb|flash|extreme|portable.*drive|hard.*drive)\b/i, tier: TIER.HIGH },
...
function classifyDevice(device) {
  ...
  for (const rule of NAME_RULES) {
    if (rule.pattern.test(name)) {
      return { ...device, tier: rule.tier, tierReason: `name match: ${rule.pattern}` };
    }
  }
  const typeTier = TYPE_MAP[device.type];
  if (typeTier) {
    return { ...device, tier: typeTier, tierReason: `type: ${device.type}` };
  }
  return { ...device, tier: TIER.MEDIUM, tierReason: "unknown – defaulted to MEDIUM" };
}
