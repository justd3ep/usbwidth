...
async function detectLinux() {
  const { getSystemInfo, getUsbDevices } = await import("./linuxDetector-DUDdQeQm.js");
  const [systemInfo, usbDevices] = await Promise.all([
    getSystemInfo(),
    getUsbDevices()
  ]);
...
async function detectHardware() {
  if (process.platform === "win32") {
    try {
      ...
      console.error("[HardwareDetector] Windows/WMI failed:", err.message);
  ...
  if (process.platform === "linux") {
    try {
      ...
      const mock = getMockData();
      mock.error = `Linux hardware read failed: ${err.message}`;
      return mock;
    }
  }
  console.log("[HardwareDetector] Unsupported platform, using mock data.");
  return getMockData();
}
export {
  detectHardware
